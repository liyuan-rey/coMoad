...
//            DynaMOAD, ClientUtil, MainFrame, LogIn, 
//            UIManagement, SearchCondition, UIBuilder, UIGeneration, 
    ...
    implements ActionListener, MouseListener
{
        ...
        private void maybeShowPopup(MouseEvent e)
        {
            if(e.isPopupTrigger() && !e.isControlDown())
            {
                ...
                try
                {
                    ...
                    popup = UIBuilder.createPopupMenuForSearchResult(instanceOuid, ouidList, assoList, e.getComponent(), e.getX(), e.getY(), SearchResult.this, modelOuid);
                }
    ...
    public SearchResult(JFrame parentFrame)
    {
        wfm = null;
        clientUtil = null;
        popup = null;
    ...
    public void initialize()
    {
        ...
        {
            ((MainFrame)parentFrame).searchResultFrame.setContent(this);
            ((MainFrame)parentFrame).searchResultFrame.setTitle(DynaMOAD.getMSRString("WRD_0046", "Search Result", 3));
        }
    }
    ...
    public void makeSearchResultTable(String lcClassOuid)
    {
        try
        {
            ...
            initialize();
            if(parentFrame instanceof MainFrame)
                ((MainFrame)parentFrame).searchCondition.setSearchResultField(fieldOuidList);
        }
        catch(IIPRequestException ie)
        {
            ie.printStackTrace();
    ...
    public void makeSearchResultTableForWorkSpace(String lcClassOuid)
    {
        try
        {
            ...
            if(classOuidForWorkSpace.equals("Favorites") || classOuidForWorkSpace.equals("Recent"))
            {
                ...
                if(parentFrame instanceof MainFrame)
                    ((MainFrame)parentFrame).searchResultFrame.setTitle(DynaMOAD.getMSRString("WRD_0046", "Search Result", 3) + " (" + searchData.size() + ")");
            } else
            if(classOuidForWorkSpace.equals("Inbox"))
            {
                if(searchData.size() > 0)
    ...
    public void setResultData(ArrayList resultData)
    {
        ...
        if(parentFrame instanceof MainFrame)
        {
            int pageNum = ((MainFrame)parentFrame).searchCondition.getPageNumber();
            int cntPerPage = ((MainFrame)parentFrame).searchCondition.getCountPerPage();
            if(resultData == null)
            {
                ((MainFrame)parentFrame).searchResultFrame.setTitle(DynaMOAD.getMSRString("WRD_0046", "Search Result", 3) + " (0)");
                ((MainFrame)parentFrame).setEnabledPreviousSearchButton(false);
                ((MainFrame)parentFrame).setEnabledNextSearchButton(false);
            } else
            {
                ((MainFrame)parentFrame).searchResultFrame.setTitle(DynaMOAD.getMSRString("WRD_0046", "Search Result", 3) + " (" + ((pageNum - 1) * cntPerPage + 1) + " ~ " + ((pageNum - 1) * cntPerPage + resultData.size()) + ")");
                if(cntPerPage <= 0 || pageNum == 1)
                    ((MainFrame)parentFrame).setEnabledPreviousSearchButton(false);
                else
                    ((MainFrame)parentFrame).setEnabledPreviousSearchButton(true);
                if(cntPerPage <= 0 || resultData.size() < cntPerPage)
                    ((MainFrame)parentFrame).setEnabledNextSearchButton(false);
                else
                    ((MainFrame)parentFrame).setEnabledNextSearchButton(true);
            }
        }
    }
        ...
        try
        {
            ...
            if(command.startsWith("MNU_DELETE"))
            {
                ...
                try
                {
                    ...
                    if(res != 0)
                        return;
                    if((parentFrame instanceof MainFrame) && ((MainFrame)parentFrame).searchCondition.isPrivateWorkSpace)
                        if(classOuidForWorkSpace.equals("Recent"))
                        {
                    ...
                    if(!(returnObject instanceof Boolean) || Utils.getBoolean((Boolean)returnObject))
                    {
                        ...
                        if(parentFrame instanceof MainFrame)
                            ((MainFrame)parentFrame).searchCondition.searchButton_actionPerformed(actionEvent);
                    }
                }
                catch(IIPRequestException ie)
                {
