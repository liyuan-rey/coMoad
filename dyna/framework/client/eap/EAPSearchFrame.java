...
// Source File Name:   EAPSearchFrame.java
...
// Referenced classes of package dyna.framework.client.eap:
//            EAPUtils

public class EAPSearchFrame extends JFrame
    implements ActionListener, WindowListener, MouseListener, DynaTableCellColor
{

    public EAPSearchFrame()
    {
        ...
        try
        {
            ...
            if(packageOuid == null)
                return;
            DOSChangeable dosClass = dos.getClassWithName(packageOuid, "ChangeOrder");
            if(dosClass == null)
                return;
            classOuid = (String)dosClass.get("ouid");
            classInfo = dos.getClass(classOuid);
            dosClass = dos.getClassWithName(packageOuid, "ApplyScheduleTemplate");
            if(dosClass == null)
                return;
    ...
    private void initComponents()
    {
        ...
        setTitle(DynaMOAD.getMSRString("INF_B4O1", "EO Apply Process Search", 0));
    ...
    private void initTemplate()
    {
        try
        {
            ArrayList tempList = dos.list(templateClassOuid);
            if(tempList == null)
                return;
            templateOuid = (String)((ArrayList)tempList.get(0)).get(0);
        }
        catch(IIPRequestException ie)
        {
            ie.printStackTrace();
    ...
    public void makeSearchResultTable()
    {
        try
        {
            if(templateOuid == null)
                return;
            eapList = EAPUtils.getColumnsOfEAPSearch(templateOuid);
            searchData = new ArrayList();
            columnName = new ArrayList();
            columnWidth = new ArrayList();
            DOSChangeable selectedResult = new DOSChangeable();
            int size = 2;
            if(!Utils.isNullArrayList(eapList))
                size += eapList.size();
            columnSequence = new int[size];
            for(int i = 0; i < size; i++)
                columnSequence[i] = i + 1;

            columnName.add("EO " + DynaMOAD.getMSRString("WRD_0020", "Number", 0));
            columnWidth.add(new Integer(80));
            columnName.add("EO " + DynaMOAD.getMSRString("WRD_0021", "Name", 0));
            columnWidth.add(new Integer(140));
            for(int i = 0; i < size - 2; i++)
            {
                columnName.add(((ArrayList)eapList.get(i)).get(1));
                columnWidth.add(new Integer(80));
            }
    ...
    public void makeSearchConditionPanel()
    {
        try
        {
            ...
            projectIdLabel = new JLabel("EO " + DynaMOAD.getMSRString("WRD_0020", "Number", 0));
            projectIdLabel.setBounds(10, 10 + 23 * (i + 2), 100, 20);
            leftPanel.add(projectIdLabel);
            eoNumberText = new JTextField();
            eoNumberText.setBounds(110, 10 + 23 * (i + 2), 180, 20);
            leftPanel.add(eoNumberText);
            i++;
            projectNameLabel = new JLabel("EO " + DynaMOAD.getMSRString("WRD_0021", "Name", 0));
            projectNameLabel.setBounds(10, 10 + 23 * (i + 2), 100, 20);
            leftPanel.add(projectNameLabel);
            eoNameText = new JTextField();
            eoNameText.setBounds(110, 10 + 23 * (i + 2), 180, 20);
            leftPanel.add(eoNameText);
            i++;
    ...
    public void actionPerformed(ActionEvent evt)
    {
        ...
        if(command.equals("search"))
            try
            {
                ...
                String searchSQL = EAPUtils.makeSQLForEAPSearch(eapList, eoNumberText.getText(), eoNameText.getText(), status.getOuid());
                if(Utils.isNullString(searchSQL))
                    return;
                ArrayList tempList = EAPUtils.eapSearch(searchSQL);
    ...
    private JTextField eoNumberText;
    private JLabel projectNameLabel;
    private JTextField eoNameText;
